...
class Game:
    ...
    def update(self):
        ...
        if self.soldierTimer == 0:
            self.soldierTimer = SOLDIER_SPAWN_TIMER
            s = Soldier(random.randint(int(p.pos.x + 600), int(p.pos.x + 800)),
    ...
    # Bắt sự kiện của nhảy bắn và phím mouse
    def events(self):
    ...
    def show_game_over_screen(self):
        ...
        if not self.bosses:
            text_to_display = "Time (" + str(int(self.time / FPS)) + " s)"
        self.draw_text(text_to_display, 21, WIDTH / 2, HEIGHT / 2 - 20)
        self.draw_text("Space to restart, Q to quit", 21, WIDTH / 2 + 10, HEIGHT / 2)
